...
namespace AtCoderAnswer
{
	class A_Frog1
	{
		static void Main(string[] args)
		{
			...
			{
				// 1個前の足場からジャンプした場合のコスト
				int prev1cost = int.MaxValue;
				if (0 <= i - 1)
				{
					int jumpcost = Math.Abs(costs[i] - costs[i - 1]);
					prev1cost = dp[i - 1] + jumpcost;
				}

				// 2個前の足場からジャンプした場合のコスト
				int prev2cost = int.MaxValue;
				if (0 <= i - 2)
				{
					int jumpcost = Math.Abs(costs[i] - costs[i - 2]);
					prev2cost = dp[i - 2] + jumpcost;
				}

				// 最小コスト確定
				if (prev1cost != int.MaxValue || prev2cost != int.MaxValue)
				{
					dp[i] = Math.Min(prev1cost, prev2cost);
				}
			}
